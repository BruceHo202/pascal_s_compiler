...
FILE * inptr = fopen ("./MarkTable.txt", "r"); //原符号表
FILE * mergeptr = fopen("./MergedMarkTable.txt", "w+"); //合并后的符号表
FILE * updateptr = fopen("./UpdatedMarkTable.txt", "w+"); //原符号表,const、type替换后，加上指向
FILE * stdoutptr = fopen("./Fstdout.txt", "w+"); //词法分析后的程序
...
void CodeLayer(){ //划分作用域
    ...
    for(int i = 1; i <= oMarkNum; i++){
        ...
        if((tp >= 1 && tp <= 4) || tp == caseType) { // program,function,procedure,record
            sta.push(WorkSpace{i, -1, ++dep, tp, 0, 0}); 
        }
        else if(tp == beginType){
            sta.push(WorkSpace{i, -2, dep, tp, 0, 0});
        }
        else if(tp == endType){
            if(sta.top().End == -2){//碰见begin。
                sta.pop();
    ...
    for(int i = 1; i <= workSpaceNum; i++){ //找父作用域
        ...
        for(int j = 1; j <= workSpaceNum; j++)if(i != j){ //遍历所有找父作用域
            ...
            if(ws2.Begin < ws1.Begin && (ws2.End > ws1.End || ws1.End == oMarkNum) && ws2.End - ws2.Begin < MinNum && ws2.depth == ws1.depth - 1){
    ...
    for(int i = 1; i <= workSpaceNum; i++){ //每个符号赋工作域
        ...
        for(int j = ws.Begin; j <= ws.End; j++){
            if(workSpace[Belong[j]].depth < ws.depth){
                Belong[j] = i;
            }
        }
    }
    ...
    //     printf("%d (%d %d) kind = %d depth = %d root = %d\n", i, workSpace[i].Begin, workSpace[i].End, workSpace[i].kind, workSpace[i].depth, workSpace[i].root);
    // }
}
...
void ReplaceConstType(){ //const、type替换，符号表合并
    ...
    for(int i = 1; i <= oMarkNum; i++){
        ...
        if(tpNow == functionType || tpNow == programType){
            int tp2 = GetType(oMark[pos - 1].mark, oMark[pos - 1].value);
            if(tp2 != functionType && tp2 != programType) tpNow = varType;
        }
...
void ReplaceConstTypeAndMerge(){ //const、type替换，符号表合并
    ...
    for(int i = 1; i <= mMarkNum; i++){
        fprintf(mergeptr, "i:%d value:%s  pos:%d  kind:%d size = %d\n", i,mMark[i].value, mMark[i].pos, mMark[i].kind, mMark[i].size);
    }
...
int main(){
    ...
    CodeLayer(); //划分作用域
    // ReplaceConstAndType(); //替换const和type
    ...
    ofstream fout1("./MergedMarkTable.dat", ios::binary);
    ...
    ofstream fout2("./UpdatedMarkTable.dat", ios::binary);
